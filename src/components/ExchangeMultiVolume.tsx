...
export default function MultiVolumeChart({
  ids,
  names,
  title,
}: VolumeChartProps) {
  ...
  async function getVolume(ids: string[], names: string[]) {
    ...
    const v_data0: [number, number][] = response0.data.map((id) => [
      id[0],
      parseInt(id[1]),
    ])
    const v_data1: [number, number][] = response1.data.map((id) => [
      id[0],
      parseInt(id[1]),
    ])
    const v_data2: [number, number][] = response2.data.map((id) => [
      id[0],
      parseInt(id[1]),
    ])
    setSeries([
      { data: v_data0, name: names[0] }, // this is a kluge and not acceptible for the real world
  ...
  if (loaded === true) {
    return (
      <>{series && <LineChart series={series} title={title}></LineChart>}</>
    )
  } else {
    return (
      <>
