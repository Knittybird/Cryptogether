      ...
      <div className='exchangeList-container'>
        <div className='exchangeList-overview'>
          <div className='total-trade-volume-24h'>
            ...
            <div className='value'>
              {' '}
              $<SimpleNum value={totalTradingVolum} />
            </div>
          </div>
        ...
        <table className='exchangeList table'>
          <tbody>
            <tr key={0}>
              <th key={0} className='d-none d-sm-table-cell'>
                #
              </th>
              <th key={1} className='text-end'>
                Coin
              </th>
              <th key={2} className='text-end'>
                Pair
              </th>
              <th key={3} className='d-none d-sm-table-cell'>
                Last Price
              </th>
              <th key={4} className='d-none d-sm-table-cell'>
                Spread
              </th>

              <th key={7} className='text-end'>
                24h Volume
              </th>
              <th key={8} className='d-none d-lg-table-cell'>
                Last Traded
              </th>
              <th key={9} className='d-none d-md-table-cell'>
                Trust Score
              </th>
            </tr>
            {tickers.map((ticker, i) => (
              <tr key={i + 1}>
                <td key={0} className='d-none d-sm-table-cell'>
                  {i + 1}
                </td>
                <td key={1} className='text-end'>
                  <Link
                    className='base-coin'
                    to={'/coin/' + ticker.coin_id}
                  >
                    {ticker.base}
                  </Link>
                </td>
                ...
                <td key={4} className='d-none d-sm-table-cell'>
                  <SimplePercentage
                    value={ticker.bid_ask_spread_percentage}
                  />
                </td>
                <td key={7} className='text-end'>
                  $<SimpleNum value={ticker.converted_volume.usd} />
                </td>
                <td key={8} className='d-none d-lg-table-cell'>
                  {new Date(ticker.last_traded_at).toLocaleString()}
                </td>
                <td key={9} className='d-none d-md-table-cell'>
                  {ticker.trust_score === 'green' ? (
                    <span className='trust-score-green'>good</span>
                  ) : ticker.trust_score === 'yellow' ? (
                    <span className='trust-score-yellow'>fair</span>
                  ) : (
                    <span className='trust-score-red'>poor</span>
                  )}
                </td>
              </tr>
            ))}
